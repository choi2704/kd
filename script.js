...
function animatePrice(to){
  ...
  function step(now){
    ...
    totalEl.textContent=won(from+(to-from)*eased);
...
function calc(){
  const w=Number(widthEl.value),h=Number(heightEl.value),asm=assembleEl.checked;
  const wp=priceOf('width',w),hp=priceOf('height',h),ap=asm?KD_PRICE.assemble:0;
  const total=KD_PRICE.base+wp+hp+ap;
  animatePrice(total);
